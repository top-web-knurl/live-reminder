...
const ICONS = {
  ...
  archiveBox: '<svg width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><rect width="20" height="5" x="2" y="3" rx="1"/><path d="M4 8v11a2 2 0 0 0 2 2h12a2 2 0 0 0 2-2V8"/><path d="M10 12h4"/></svg>',
...
document.addEventListener('DOMContentLoaded', async () => {
  ...
  document.getElementById('remindersIcon').innerHTML = ICONS.bell;
  document.getElementById('templatesIcon').innerHTML = ICONS.copy;
  document.getElementById('archiveIcon').innerHTML = ICONS.archiveBox;
  await loadReminders();
});
...
function initializeSearch() {
  ...
  const debouncedSearch = debounce((query) => {
    ...
    clearBtn.classList.toggle('visible', !!currentSearchQuery);
...
async function loadSettings() {
  try {
    const result = await window.dbAPI.getAutostart();
    document.getElementById('autostartToggle').checked = result.enabled;
  } catch (error) {
  ...
  document.getElementById('autostartToggle').addEventListener('change', async (e) => {
    try {
      await window.dbAPI.setAutostart(e.target.checked);
    } catch (error) {
      console.error('Set autostart error:', error);
  ...
  document.getElementById('exportDbBtn').addEventListener('click', async () => {
    try {
      const result = await window.dbAPI.exportDb();
      if (result.success) {
        showMessage('База данных экспортирована!', 'success');
      } else if (result.canceled) {
        return;
      } else {
        showMessage('Ошибка экспорта: ' + result.message, 'error');
      }
    } catch (error) {
  ...
  document.getElementById('importDbBtn').addEventListener('click', async () => {
    try {
      ...
      if (result.success) {
        showMessage('Импорт завершён! Приложение перезагрузится...', 'success');
        setTimeout(() => location.reload(), 1500);
      } else if (result.canceled) {
        return;
      } else {
        showMessage('Ошибка импорта: ' + result.message, 'error');
      }
    } catch (error) {
